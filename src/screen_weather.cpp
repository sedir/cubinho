...
static void drawWeatherIcon(lgfx::LovyanGFX& d, int code, int cx, int cy) {
    if (code == 0) { drawSun(d, cx, cy, 13); }
    else if (code >= 1 && code <= 3) { drawSun(d, cx - 8, cy - 8, 9); drawCloud(d, cx + 4, cy + 4, COLOR_CLOUD); }
    ...
    else if (code >= 51 && code <= 57) { drawCloud(d, cx, cy - 8, COLOR_CLOUD); drawRain(d, cx, cy + 8, 3); }
    else if ((code >= 61 && code <= 67) || (code >= 80 && code <= 82)) { drawCloud(d, cx, cy - 8, COLOR_CLOUD_DARK); drawRain(d, cx, cy + 8, 5); }
    else if ((code >= 71 && code <= 77) || (code >= 85 && code <= 86)) { drawCloud(d, cx, cy - 8, COLOR_CLOUD); drawSnow(d, cx, cy + 8, 3); }
    else if (code >= 95) { drawCloud(d, cx, cy - 8, COLOR_CLOUD_DARK); drawBolt(d, cx, cy + 8); }
    else { drawCloud(d, cx, cy, COLOR_CLOUD); }
}
...
static uint16_t tempColor(float t) {
    if (t < COMFORT_TEMP_MIN) return COLOR_TEMP_COLD;
    if (t > COMFORT_TEMP_MAX) return COLOR_TEMP_HOT;
...
static void drawHourlyForecast(lgfx::LovyanGFX& d, const WeatherData& data) {
    ...
    const int BAR_W = 36, GAP = 6, BAR_MAX = 16, BAR_MIN = 4;
    const int BASE_Y = 192;
...
void screenWeatherDraw(lgfx::LovyanGFX& display, const WeatherData& data, bool fetching) {
    ...
    display.drawFastHLine(10, 88, display.width() - 20, COLOR_DIVIDER);
    ...
    int x2 = drawTempC(display, "Max: ", data.tempMax, 0, 16, 114, COLOR_TEXT_PRIMARY);
    drawTempC(display, "  Min: ", data.tempMin, 0, x2, 114, COLOR_TEXT_PRIMARY);
    ...
    display.drawString(buf, 6, 207);

    drawBatteryWarning(display, 210);
    drawScreenIndicator(display, 1, SCREEN_COUNT);
}
