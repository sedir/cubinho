...
static void processCommand(const char *cmd, bool toTelnet)
{
    ...
    if (strcmp(cmd, "ls") == 0)
    {
        if (!_sdAvailable)
        {
            out("[sd] Sem cartao SD");
            return;
        }
        File dir = SD.open("/logs");
        if (!dir)
        {
            out("[sd] Falha ao abrir /logs");
            return;
        }
        out("--- /logs/ ---");
    ...
    else if (strncmp(cmd, "cat ", 4) == 0)
    {
        ...
        while (f.available())
        {
            ...
            if (ch >= 0 && toTelnet && _client.connected())
                _client.write((uint8_t)ch);
        }
        f.close();
        out("\n--- fim ---");
    }
    ...
    else if (strcmp(cmd, "event list") == 0 || strcmp(cmd, "events") == 0)
    {
        ...
        for (int i = 0; i < n; i++)
        {
            ...
            if (e)
                out("  %d. %s  %02d/%02d %02d:%02d", i, e->name, e->day, e->month, e->hour, e->minute);
        }
        out("--- fim ---");
    }
    else if (strncmp(cmd, "event del ", 10) == 0)
    {
        int idx = atoi(cmd + 10);
        if (eventsRemove(idx))
            out("Evento %d removido", idx);
        else
            out("Indice invalido: %d", idx);
    }
